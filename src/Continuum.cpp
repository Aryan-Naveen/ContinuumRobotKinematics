...
#include "continuum_robot/eigenmatrixtypes.h"
#include <limits>
...
void Continuum::setSegmentBasePose(int segID, tf::Vector3 basePos, tf::Quaternion baseRot){
	...
	double Continuum::getIncrement(double val, double incr_scale, double incr_floor){
	...
	void Continuum::getJacobian(Matrix6d& J,
	                        Vector6d& y,
	                        Vector6d& r,
	                        double incr_scale = 1e-8,
	                        double incr_floor = 1e-11){
			vector<double> thresholds {1/segmentLength[0], DBL_MAX, 1/segmentLength[1], DBL_MAX, 1/segmentLength[2], DBL_MAX};
	    for(int i = 0; i < y.size(); i++){
	        ...
	        y(i) += incr;
					if (y(i) > thresholds[i]) {
 						r_ = forwardKinematics(y);
					}else{
						r_ = r;
					}
					J.col(i) = (r_ - r) / incr; // + O(incr)
	        y(i) = temp;
	    }
	}

// [kappaseg1, phiseg1, kappaseg2, phiseg2, kappaseg3, phiseg3]
Vector6d Continuum::forwardKinematics(Vector6d theta){
	...
	for(int segID=0; segID < 3; segID++){
			...
			std::cout << segID << " ~~~~~~~~ " << tf::Matrix3x3(basePose[segID].getRotation()).getRow(0).getX() <<  std::endl;;
...
Vector6d Continuum::inverseKinematics(Vector6d &thguess, Vector6d &targetOrientation){
		Vector6d pose = forwardKinematics(thguess);
		Vector6d error = targetOrientation - pose;
		int count = 0;
		while(error.squaredNorm() > 1e-6 && count < 100){
			Matrix6d J = Matrix6d::Zero(6, 6);
			getJacobian(J, thguess, pose);
			Matrix6d J_pinv = J.completeOrthogonalDecomposition().pseudoInverse();
			thguess += J_pinv*error;
			FullPivLU<MatrixXd> lu(J);
			std::cout << lu.kernel();
			pose = forwardKinematics(thguess);
			error = targetOrientation - pose;
			count += 1;
			std::cout << error.squaredNorm() << "| " << count << "\n";
		}
		thguess(0) = count;
		return thguess;
}
...
void Continuum::setSegmentShape(int segID, double kappa, double phi){
	double thetas[] = {kappa, phi, kappa, phi, kappa, phi};
	Map<Vector6d> theta(&thetas[0]);
